...
public class Swerve50HzOdometry implements SwerveOdometry {
    ...
    public Swerve50HzOdometry(OdometryConfig Config) {
        ...
        // skidDetectedLog = new LoggedBool("/Subsystems/Swerve/Odometry/Skid
        // Detected");
        // collisionDetectedLog = new LoggedBool("/Subsystems/Swerve/Odometry/Collision
        // Detected");
        // lastSkidLog = new LoggedDouble("/Subsystems/Swerve/Odometry/Last Skid");
        // lastCollisionLog = new LoggedDouble("/Subsystems/Swerve/Odometry/Last
        // Collision");//UPDATE LOG
        // UPDATE LOG

        skidDetector = new SkidDetector(SwerveConstants.kinematics,
                () -> SwerveSubsystem.getInstance().getSwerveModuleStates());
    ...
    public void updateOdometry() {
        ...
        if (((skidDetected && odometryConfig.updateInSkid) || !skidDetected)
                && ((collisionDetected && odometryConfig.updateInCollision) || !collisionDetected)) {
            PoseEstimator.getInstance().updateOdometry(swerveSubsystem.getSwerveModulePositions(),
                    swerveSubsystem.getRotation2d(), Timer.getFPGATimestamp());
        }
    }
