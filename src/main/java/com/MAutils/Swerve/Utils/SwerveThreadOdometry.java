...
public class SwerveThreadOdometry {
    ...
    public void updateOdometry() {
     ...
     collisionDetected = collisionDtector.getForceVectorSize() > config.collisionForce;
        ...
        // if (!RobotConstants.FieldRectangle.contains(RobotContainer.poseEstimator.getEstimatedRobotPose().getTranslation()) || RobotConstants.BLUE_REEF_CIRCLE.contains(RobotContainer.poseEstimator.getEstimatedRobotPose().getTranslation())
        // || RobotConstants.RED_REEF_CIRCLE.contains(RobotContainer.poseEstimator.getEstimatedRobotPose().getTranslation())) {
        //     includeMeasurement = false;
        // }

        if (!RobotConstants.FieldRectangle.contains(RobotContainer.poseEstimator.getEstimatedRobotPose().getTranslation()) && 
        !RobotConstants.FieldRectangle.contains(getLookAhedPose(1).getTranslation())) {
            includeMeasurement = false;
        }

        if (Math.abs(swerveSubsystem.getGyroData().getRoll())> 20 || Math.abs(swerveSubsystem.getGyroData().getPitch())>7 ){
            includeMeasurement = false;
        }
    ...
    public Pose2d getLookAhedPose(double time) {
        speeds = TeleopSwerveController.driveController.getFieldRelativsSpeeds();
        lookAhedPose = RobotContainer.poseEstimator.getEstimatedRobotPose().plus(new Transform2d(
                speeds.vxMetersPerSecond * time, speeds.vyMetersPerSecond * time, Rotation2d.kZero));
        return lookAhedPose;
    }
