...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public boolean isStall() {
    ...
    for (int i = 0; i < modulesArry.length; i++) {
      if (Math.abs(modulesData[i].getDriveVelocity()) < 0.1 && Math.abs(modulesData[i].getDriveVolts()) > 0.2
      && Math.abs(modulesData[i].getDriveCurrent()) > 1 ) {
        stallNum++;
      }
    }
